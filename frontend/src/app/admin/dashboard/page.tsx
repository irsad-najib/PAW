    ...
    fetchOrderSummary(todayISO)
      .then((res) => {
        ...
        if (res.orders && Array.isArray(res.orders) && res.orders.length) {
          ...
          const mealGroups = Array.from(mealTimeMap.entries()).map(
            ([mealTime, mealData]) => {
              const items = Array.from(mealData.menuMap.entries()).map(
                ([name, data]) => ({
                  name,
    ...
    fetchOrderSummary(tomorrowISO)
      .then((res) => {
        ...
        if (res.orders.length) {
          const mealTimeMap = new Map<
            string,
            Map<string, { portions: number; notes: string[] }>
          >();
          ...
          const mealGroups = Array.from(mealTimeMap.entries()).map(
            ([mealTime, menuMap]) => {
              const items = Array.from(menuMap.entries()).map(
                ([name, data]) => ({
                  name,
                  portions: data.portions,
                  notes: data.notes,
    ...
    <div>
      ...
      <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg mb-8">
        ...
        <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
          <ProductionSummary
            mealData={todayMeal}
            onOpenNotes={(label, notes) =>
              setNotesModal({ title: label, notes })
            }
      ...
      <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg mb-8">
        <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4">
          Ringkasan Pesanan Besok (
          {formatDateLongID(new Date(Date.now() + 24 * 60 * 60 * 1000))})
        </h2>
        <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
